...
DETECTION_MIN_CONFIDENCE = 0.7, GPU_COUNT=1)
            ...
            while True:
                ...
                if ret:
                    ...
                    if mask_points_values == False:
                        ...
                        output = cv2.resize(output, (height, width), interpolation=cv2.INTER_AREA)
...
class custom_segmentation:
    ...
    def inferConfig(self, network_backbone = "resnet101", num_classes = 1, class_names = ["BG"], batch_size = 1, detection_threshold = 0.7, gpu_count=1):
        self.config = Config(BACKBONE = network_backbone, NUM_CLASSES = 1 + num_classes, class_names = class_names, DETECTION_MIN_CONFIDENCE = detection_threshold,
                             IMAGES_PER_GPU=batch_size, GPU_COUNT=gpu_count)
    ...
    def process_capture(self, capture, show_bboxes = False, mask_points_values = False, output_video_name = None, frames_per_second = None):
        #capture = cv2.VideoCapture(video_path)
        width = int(capture.get(cv2.CAP_PROP_FRAME_WIDTH))
        height = int(capture.get(cv2.CAP_PROP_FRAME_HEIGHT))
        codec = cv2.VideoWriter_fourcc(*'DIVX')
...
#ysc
def display_object(image, boxes, masks, class_ids):
